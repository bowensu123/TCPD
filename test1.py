...
# -*- coding: utf-8 -*-
"""
NPU VAE Encoder Tiled Encoding Memory Sweep
==========================================

This script measures **peak NPU memory** for different tiling parameters when
encoding images with a VAE encoder (e.g., Diffusers AutoencoderKL) and helps you
find the **lowest-memory configuration**.

Features
- Tests baseline (no tiling) vs tiled encoding (`encode_tiled`).
- Grid-search over tile sizes, halos, dtypes, pad modes, channels_last.
- Robust memory measurements on NPU (Ascend / torch-npu).
- Handles OOM gracefully and continues.
...
python npu_vae_mem_test.py --model-id stabilityai/sd-vae-ft-mse \
  --H 1536 --W 1536 --batch 1 --stride-total 8 --save-csv results.csv

# 2) With your own module exposing build_vae():
python npu_vae_mem_test.py --vae-script /path/to/my_vae.py \
  --H 1024 --W 1024 --batch 1

# 3) Fallback DummyVAE (for sanity check only):
python npu_vae_mem_test.py --use-dummy --H 1024 --W 1024

Notes
-----
- For best allocator behavior on NPU, consider setting before running:
  export PYTORCH_NPU_ALLOC_CONF="max_split_size_mb:256"
- Peak memory is measured via torch.npu (if available); reserves and allocated are reported.
- If you only care about **minimum memory**, start with smaller tile sizes and BF16.
"""
...
def encode_tiled(
    ...
    *,
    ...
    """
    Tile + halo VAE encoder for NPUs.
    Returns latents of shape (N, C_latent, H//s_ceil, W//s_ceil).
    """
    ...
    if device is None:
        device = x.device if (hasattr(x, "device") and x.device.type == "npu") else torch.device("npu")
    x = x.to(device)
    if channels_last:
    ...
    def _encode_patch(patch: torch.Tensor) -> torch.Tensor:
        ...
        cm = torch.autocast(device_type="npu", dtype=dtype)
...
class DummyVAE(nn.Module):
    ...
    def __init__(self, in_ch=3, lat_ch=4, stride_total=8):
        super().__init__()
        # two stride-2 blocks + one stride-2 conv => 8x downsample
...
def npu_max_allocated() -> Optional[int]:
    if hasattr(torch, "npu") and hasattr(torch.npu, "max_memory_allocated"):
        return int(torch.npu.max_memory_allocated())
    return None

def npu_max_reserved() -> Optional[int]:
    if hasattr(torch, "npu") and hasattr(torch.npu, "max_memory_reserved"):
        return int(torch.npu.max_memory_reserved())
    return None
...
def run_trial(vae: nn.Module, x: torch.Tensor, cfg: TrialCfg) -> TrialResult:
    assert x.device.type == "npu", "Input must be on NPU."
    # Prepare input per cfg
    xin = x
    ...
    try:
        ...
        peak_alloc = npu_max_allocated() or 0
        peak_reserved = npu_max_reserved() or 0
        # Cleanup
        del out
        gc.collect()
        npu_empty_cache()
        ...
        # Likely OOM; record and continue
        npu_synchronize()
        peak_alloc = npu_max_allocated() or 0
        peak_reserved = npu_max_reserved() or 0
        gc.collect(); npu_empty_cache()
        return TrialResult(
            mode=cfg.mode,
            tile=cfg.tile,
            halo=cfg.halo,
...
def obtain_vae(args) -> nn.Module:
    ...
    if args.model_id:
        try:
            ...
            vae = AutoencoderKL.from_pretrained(args.model_id, subfolder=getattr(args, "subfolder", "vae"))
...
def parse_args():
    p = argparse.ArgumentParser(description="NPU VAE tiled encoding memory sweep")
    p.add_argument("--model-id", type=str, default=None, help="Diffusers model id or local path containing VAE (subfolder=vae)")
    ...
    p.add_argument("--dtypes", type=str, default="bf16,fp16", help="Comma list: bf16,fp16,fp32 (bf16 recommended on NPU)")
...
def main():
    ...
    if args.use_dummy:
        args.model_id = None
        args.vae_script = None

    assert torch.device("npu") is not None, "NPU device not available."
    ...
    if args.save_csv:
        fieldnames = list(asdict(ok_results[0]).keys()) if ok_results else list(asdict(results[0]).keys())
